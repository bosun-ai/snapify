...
Git-cliff + the `Prepare Release` workflow watch Conventional Commits to draft release PRs (`release/snapify-vX.Y.Z`). Keep subjects scoped (`feat(core): add section blocks`, `fix(cli): guard missing template`) so `conventional-recommended-bump` can infer the right semver. When touching baselines, link the issue, describe artifact diffs, and attach screenshots reviewers can inspect. Always list the verification commands you ran (`npm test`, `npm run build`) in the PR description. Only merge release PRs as-is; doing so tags `snapify-vX.Y.Z`, creates the GitHub release, and triggers npm publishing.
